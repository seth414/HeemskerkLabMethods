function G = create_tracks(this, opts)
%%%% Inputs %%%%
...
%%%%%% TODO: check that dejittered positions are actually used for the
%%%%%% frame-frame linking step %%%%%%%
if ~opts.dejitter
...
T = cell(size(points));
nlinks = 0;
%for huge datasets, parallel for loop causes the computer to run out of
%memory; can I write this in a smarter way so that doesn't happen?
%try not passing full positions object to the framelinker function each
%time?
parfor ti=1:tmax - 1
...
%splitting, merging, gap-closing step
[assignments, costs, ~, ~] = graphtracklinker(G, max_gap_time,...
    max_gap_distance, points, areas, Divs, angles);
%put the new linking information in a convenient format and add edges
...
[newedges,badedges,costs] = graphmergelinker_v2(G,points,areas,...
    Divs,max_gap_time);
...
%additional splitting step
[assignments, costs, ~, ~] = tracksplitter(G, max_gap_time,...
    max_gap_distance*2, points, areas, Divs);
