...
%                         LiveFixedLinkerWithZ(source_info, target_info, imsize, max_distance);
                    ...
                    if isempty(target_info) || isempty(source_info)
                        source_indices = -1*ones(1,size(target_info,1));
                    else
        ...
        function checkAlignment(this, pidx, opts)
...
%             fixedIm = imadjust(fixedIm,stitchedlim(fixedIm));
            ...
            if ~isempty(p_fixed.cellData(1).XY) && ~isempty(p_live.cellData(end).XY)
    ...
    %             XYlive = p_live.cellData(p_live.nTime).XY;
                ...
                scatter(XYfixed(:,1),XYfixed(:,2),opts.markersize,'w','filled');
                scatter(XYfixed(:,1),XYfixed(:,2),opts.markersize*0.75,'k','filled');
                scatter(XYlive(:,1),XYlive(:,2),opts.markersize,'k','filled');
                scatter(XYlive(:,1),XYlive(:,2),opts.markersize*0.75,'w','filled');
                quiver(XYfixed(mappedSourceIdx,1),XYfixed(mappedSourceIdx,2),dXY(:,1),dXY(:,2),0,'LineWidth',opts.linewidth,...
                    'Color',lines(1),'MaxHeadSize',0.5);%[0 0.4470 0.7410]);%[0.069,0.7586,0.931]);
            end
