...
if isfield(opts,'ext') && ~isempty(opts.ext)
    ...
    exts = {'.ims','.nd2','.lif'}; %TODO: expand this list
    idx = 1;
    criterion = true;
...
if ~isfield(opts,'snakeorder') %use 'leftright' or 'bottomtop'
    snakeorder = 'leftright';
else
    snakeorder = opts.snakeorder;
end
...
if isfield(opts,'registrationmode')
...
if ~isfield(opts,'orderFromFilename')
    opts.orderFromFilename = false;
end
...
if ~montageFusionGrid
    if strcmp(snakeorder,'leftright')
        for ii = 1:gridsize(1)
    ...
    elseif strcmp(snakeorder,'bottomtop')
        for ii = 1:gridsize(2)
            if mod(ii,2) == 1 %alternate direction for odd and even rows
                columnorder = gridsize(1):-1:1;
            else
                columnorder = 1:gridsize(1);
            end
            order(idx:idx + gridsize(1) - 1,:) = [columnorder(:),ii*ones(gridsize(1),1)];
            idx = idx + gridsize(1);
        end
    end
else
...
while pidx <= ngrids %TODO: add the option to only do stitching for specified colonies/grids
    ...
    if nfiles >= pidx*ppg || seriesflag
        ...
        str1 = ['stitched',sprintf(bare,pidx-1,0,0)];
        ...
        for ti = 1:nt
            ...
            for idx = 1:ppg
                ...
                if seriesflag
                    ...
                    if ti == 1 && idx == 1
                        fprintf(fileID,(strcat(listing(1).name, "\n")));
                    end
                else
                    ...
                    if ti == 1
                        fprintf(fileID,(strcat(listing(fidx).name, "\n")));
                    end
                end
...
%                     nt = r.getSizeT; %set nt above to allow us to set tmax more easily
                ...
                ii = order(idx,1); jj = order(idx,2);
                if opts.orderFromFilename
                    [~,barename,~] = fileparts(fname);
                    s = strsplit(barename,'_');
                    jj = str2double(s{end-1}); ii = str2double(s{end});
                end
            ...
            if strcmp(registrationmode,'automatic')
                upperleft = registerImageGrid_v3(nucmips, [pixelOverlapY pixelOverlapX]);
            elseif strcmp(registrationmode,'nominal')
                warning('using nominal image offsets instead of automatic registration')
                upperleft = registerImageGrid_nominal(nucmips, [pixelOverlapY pixelOverlapX]);
            end
            ...
            for ci = 1:nc
                ...
                mipname = strrep(mipname,'.tif',mipext); %save the MIP in the specified image file type (tif, png, jpg)
...
meta.filenameFormat = ['stitched',bare];
meta.fileNames = {};
