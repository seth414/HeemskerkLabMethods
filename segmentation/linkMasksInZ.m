...
%option to read out cytoplasmic levels from one z slice or from the whole
%3D mask
if ~isfield(opts,'cytoMode')
    cytoMode = 'singleSlice';
else
    cytoMode = opts.cytoMode;
end

if strcmp(cytoMode,'singleSlice')
    disp('reading cytoplasmic levels from a single z slice')
else
    disp('using 3D cytoplasmic masks')
end
...
for jj = 1:nCells
    ...
    cytoindices = cell(size(cellinfo,1),1);
    ...
    if ~isempty(allmasks(zi).cytmask)
        ...
        elseif strcmp(cytoMode,'3D')
            masks(jj).cytmask = cell2mat(cytoindices);
        else
            error('invalid cytoMode option')
        end
    else
        masks(jj).cytmask = [];
    end
